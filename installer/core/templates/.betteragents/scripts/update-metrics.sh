...
MEMORY_DIR="$PROJECT_ROOT/.betteragents/memory"
...
# Extract session-level avg for token reduction (session baseline was 150k before Phase 1)
AVG_SESSION=$(echo "$LLM_USAGE_DATA" | jq '.totals.avgPerSession // 0')
...
if jq \
    ...
    '
    .lastUpdated                         = $now |
    .metadata.totalTokensUsed            = $d.total_tokens |
    .metadata.totalTokensRemaining       = $d.remaining |
    .metadata.utilizationPercent         = $d.utilization |
    .byAgent                             = $d.by_agent |
    .projections.estimatedRunwayDays     = (
        if $d.avg_tokens > 0
        then ($d.remaining / $d.avg_tokens | floor)
        else 999 end )
    ' "$TOKEN_ACC" > "$TMP"; then
    mv "$TMP" "$TOKEN_ACC"
    UTIL=$(echo "$COMPUTED" | jq '.utilization')
    TOTAL=$(echo "$COMPUTED" | jq '.total_tokens')
