...
GIT_CHANGES=$(cd "$PROJECT_ROOT" && git diff --name-only HEAD 2>/dev/null \
    | grep -v '\.claude/memory/' | grep -v '\.kiro/memory/' | wc -l || echo 0)
...
- Completed work → bash .kiro/scripts/add-task.sh ...
- Decisions made → bash .kiro/scripts/add-decision.sh ...
- Patterns found → bash .kiro/scripts/add-pattern.sh ...

Use 'jq .tasks .kiro/memory/progress.json' to check current state.
DEBT
