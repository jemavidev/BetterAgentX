...
# Check agents
if [ -d "$BETTERAGENTX_DIR/.claude/agents" ]; then
    for agent in "$BETTERAGENTX_DIR/.claude/agents"/*.md; do
        if [ -f "$agent" ]; then
...
if [ -d "$BETTERAGENTX_DIR/templates/hooks" ]; then
    # Hooks now managed via .claude/settings.local.json
    if file_changed "$BETTERAGENTX_DIR/.claude/settings.local.json" ".claude/settings.local.json"; then
        CHANGES["hooks"]=1
        CHANGES_DETECTED=$((CHANGES_DETECTED + 1))
    fi
fi
...
if [ -f "$BETTERAGENTX_DIR/templates/memory/dashboard.html" ]; then
    if file_changed "$BETTERAGENTX_DIR/templates/memory/dashboard.html" ".claude/memory/dashboard.html"; then
        CHANGES["dashboard"]=1
        CHANGES_DETECTED=$((CHANGES_DETECTED + 1))
    fi
fi
...
if [ "$SKIP_BACKUP" = false ]; then
    ...
    if [ "${CHANGES[hooks]}" = "1" ] && [ -d ".claude/hooks" ]; then
    ...
    if [ "${CHANGES[dashboard]}" = "1" ] && [ -f ".claude/memory/dashboard.html" ]; then
        cp .claude/memory/dashboard.html "$BACKUP_DIR/" 2>/dev/null || true
    fi
...
[ -d "$BACKUP_DIR/hooks" ] && cp -r "$BACKUP_DIR/hooks" .claude/
[ -d "$BACKUP_DIR/scripts" ] && cp -r "$BACKUP_DIR/scripts" .
[ -f "$BACKUP_DIR/dashboard.html" ] && cp "$BACKUP_DIR/dashboard.html" .claude/memory/
...
if [ "${CHANGES[hooks]}" = "1" ]; then
    ...
    if [ -d "$BETTERAGENTX_DIR/templates/hooks" ]; then
        # Claude Code hooks are in settings.local.json
        if file_changed "$BETTERAGENTX_DIR/.claude/settings.local.json" ".claude/settings.local.json"; then
            cp "$BETTERAGENTX_DIR/.claude/settings.local.json" ".claude/settings.local.json" 2>/dev/null && \
                HOOKS_UPDATED=$((HOOKS_UPDATED + 1)) && \
                print_info "Updated: settings.local.json (hooks)"
        fi

        print_success "Hooks updated ($HOOKS_UPDATED changed)"
    else
        print_info "No hooks configuration found"
    fi
    echo ""
else
...
if [ "${CHANGES[dashboard]}" = "1" ]; then
    ...
    if [ -f "$BETTERAGENTX_DIR/templates/memory/dashboard.html" ]; then
        cp "$BETTERAGENTX_DIR/templates/memory/dashboard.html" .claude/memory/dashboard.html 2>/dev/null
...
[ ! -d ".claude/memory" ] && VERIFICATION_PASSED=false
[ ! -f ".claude/.version" ] && VERIFICATION_PASSED=false
