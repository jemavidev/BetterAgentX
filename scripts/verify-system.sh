...
if [ -d "$PROJECT_ROOT/.claude/agents" ]; then
    ...
    if [ "$AGENT_COUNT" -eq 13 ]; then
        ...
        if [ "$DEBUG_MODE" = true ]; then
            ls -1 "$PROJECT_ROOT/.claude/agents"/*.md 2>/dev/null || echo "  None"
        fi
        ISSUES_FOUND=$((ISSUES_FOUND + 1))
    fi
else
    print_error "Carpeta de agentes no encontrada"
    debug "Expected path: $PROJECT_ROOT/.claude/agents"
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
fi
...
if [ -d "$PROJECT_ROOT/.claude/memory" ]; then
    ...
    if [ "$DEBUG_MODE" = true ]; then
        echo "  Memory files:"
        find "$PROJECT_ROOT/.claude/memory" -maxdepth 1 -name "*.json" -type f 2>/dev/null | while read f; do
            echo "    - $(basename $f)"
        done
    fi
...
if [ -L "$PROJECT_ROOT/.agents/skills" ]; then
    ...
    print_info "No legacy symlink (using .claude/skills directly)"
fi
...
TOTAL_SKILLS=$(grep -hc "npx skills.*add" "$PROJECT_ROOT/.claude/agents"/*.md 2>/dev/null | awk '{s+=$1} END {print s}')
...
if [ "$TOTAL_SKILLS" -gt 0 ] 2>/dev/null; then
    ...
    grep -h "npx skills" "$PROJECT_ROOT/.claude/agents"/*.md 2>/dev/null | \
...
if [ -d "$PROJECT_ROOT/.claude/skills" ]; then
    INSTALLED_SKILLS=$(ls -1 "$PROJECT_ROOT/.claude/skills" 2>/dev/null | wc -l)
...
SCRIPTS=("$PROJECT_ROOT/scripts/init.sh" "$PROJECT_ROOT/scripts/verify-system.sh" "$PROJECT_ROOT/scripts/update-skills.sh" "$PROJECT_ROOT/scripts/memory-stats.sh" "$PROJECT_ROOT/scripts/update-dashboard.sh")
SCRIPTS_OK=0
