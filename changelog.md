...
### 🎯 Focus
Universal AGENTS.md format, modular plugin-based installer, full multi-platform support (Claude Code + Kiro), and complete system audit with consistency fixes.

### ✨ Added

#### Multi-Platform Architecture
- ✅ `AGENTS.md` — universal orchestrator format (replaces platform-specific CLAUDE.md as repo root config)
- ✅ `.betteragents/` — cross-platform adapter layer (sync, translators, memory bridge)
- ✅ `.betteragents/sync/` — bidirectional sync scripts (Claude ↔ Kiro)
- ✅ `.betteragents/translators/` — format translators (claude-to-kiro.js, kiro-to-claude.js)
- ✅ `.betteragents/sync/memory-bridge.js` — cross-platform memory access
- ✅ `docs/platforms/claude.md` + `docs/platforms/kiro.md` — platform guides

#### Modular Installer
- ✅ `installer/install.sh` — plugin-based modular installer (replaces monolithic init.sh for Option A)
- ✅ `installer/lib/` — core.sh, platform-registry.sh, config-manager.sh, ui.sh
- ✅ `installer/config/platforms.json` — platform registry for extensibility
- ✅ Support for `--platform=claude|kiro|both`, `--target`, `--interactive`, `--unregister`

#### Memory Write Enforcement (3-layer)
- ✅ `add-task.sh`, `add-decision.sh`, `add-pattern.sh` — CLI helpers with validation
- ✅ Memory self-assessment gate (Protocol 5b) — autonomous post-response scoring
- ✅ Cross-session debt detection via `on-session-stop.sh` + `on-user-prompt.sh`

#### Safety Framework (6 pillars)
- ✅ Anti-loop detection (3-call checkpoint, pattern recognition)
- ✅ Plan mode enforcement (auto-trigger for ≥3 files / complexity ≥5)
- ✅ Post-change verification (syntax check by file type after every Edit/Write)
- ✅ Triviality filter (score ≥4 → suggest manual instead of burning tokens)
- ✅ Token budget per agent (context window limits per subagent_type)
- ✅ Critic gate (mandatory for all Architect decisions)

#### Dashboard
- ✅ Interactive HTML dashboard with session tracking, token usage, safety metrics
- ✅ Node.js server (`serve-dashboard.js`) — no Docker required
- ✅ Docker Compose option for multi-project central dashboard
- ✅ SAFETY tab: file heat-map, token distribution bar, session activity cards

### 🔧 Fixed (Consistency Audit 2026-03-17)
- ✅ Unified version to 4.0.0 across README, betteragents.json, .claudecode.json, installer/CLAUDE.md
- ✅ Added `workflow-prefs.md` to `templates/memory/` (was missing, referenced by MEMORY.md)
- ✅ Updated README memory table: added workflow-prefs.md, memory-stats.json, project-metrics.json, historical-data.json
- ✅ Rewrote QUICK-START.md to actually be a quick start guide (previously contained only multi-platform sync docs)
- ✅ Fixed betteragents.json internal inconsistency (metadata.version was 3.7.0 while root version was 3.8.0)
- ✅ Updated betteragents.json metadata.lastModified to 2026-03-17
...
### 🎯 Objective
Enable BetterAgents integration into any project using symbolic links without file duplication, with automatic memory activation.
...
- Multiple projects can share one BetterAgents
...
- Not uploaded to Git
- Copied from BetterAgents templates

### 📊 Benefits

- ✅ Use BetterAgents in any project
- ✅ No file duplication
- ✅ Easy updates
- ✅ Project-specific memory
- ✅ Local configurations
- ✅ Multiple projects sharing BetterAgents

### 🎯 Use Cases

1. **New Project** - Initialize with BetterAgents from start
2. **Existing Project** - Add BetterAgents to existing project
3. **Monorepo** - Use in monorepo root
4. **Multiple Projects** - Share one BetterAgents between projects
