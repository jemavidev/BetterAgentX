...
## Description
Force a memory checkpoint — saves all decisions, tasks, and context from the current session. Use during long sessions or before closing to prevent memory loss.
...
- IDs are sequential: check existing entries first (`jq '.tasks[-1].id' .claude/memory/progress.json`)
